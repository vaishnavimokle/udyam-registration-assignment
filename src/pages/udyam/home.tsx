...
import { OTPVerificationType } from "@/constants/udyam"
...
const HomePage = () => {
    ...
    useEffect(() => {
      if (udyamWindow) {
        const udyamPopupMessageListener = (event: MessageEvent) => {
          const data = event.data

          // start poller on success
          // if poller has correct stage:
          // redirect to OTP
          router.push({
            pathname: "/udyam/otp",
    ...
    const onInitiate = async () => {
        setBtnLoading(true)
        try {
            const udyamUrl = (await udyamInitiate()).data.sessionUrl;
        ...
        <div className="w-full flex flex-col justify-around pt-10 md:pt-20">
          <div className="flex flex-col gap-6 items-center">
            <div className="w-full max-w-sm">
              <Button onClick={onInitiate}>Initiate Udyam Registration</Button>
            </div>
          </div>
        </div>
      </ScreenLayout>
